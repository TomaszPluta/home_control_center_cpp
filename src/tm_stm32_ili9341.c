...
//			TM_ILI9341_DrawPixel(i, j, color);
//		}
//	}
//}
...
//
//
//	TM_ILI9341_SetCursorPosition(40, 160, 240, 320);
//	TM_ILI9341_SendCommand(ILI9341_GRAM);
//	ILI9341_WRX_SET;
//	ILI9341_CS_RESET;
//
...
//
//	RCC->AHBENR |= RCC_AHBENR_DMA1EN;
//	static uint8_t color_static2 = ILI9341_COLOR_YELLOW;
//
////	SPI2->CR2 |= SPI_CR1_DFF;
//	SPI2->CR2 |= SPI_CR2_TXDMAEN;
//
...
//	DMA1_Channel5->CNDTR = 65530;
//	DMA1_Channel5->CPAR = (uint32_t) &SPI2->DR;
//	DMA1_Channel5->CMAR = (uint32_t) &color_static2;
////	DMA1_Channel5->CMAR = DMA_CCR1_MSIZE_0;
////	DMA1_Channel5->CMAR = DMA_CCR1_PSIZE_0;
//
//
...
void glcd_fill_screen_dma (uint16_t color){
...
static void glcd_hline(unsigned int x0, unsigned int y0, unsigned int length, unsigned int color)
{
//	if(glcd_orientation == TM_ILI9341_Orientation_Portrait_1)
//	{
		TM_ILI9341_SetCursorPosition(x0, y0, x0+length, y0);
		int i=0;
		for(i=0; i<length; i++){
			TM_ILI9341_SendCommand(ILI9341_GRAM);
			TM_ILI9341_SendData(color >> 8);
...
//
//	TM_ILI9341_DrawLine(x0, y0, x1, y1, ILI9341_COLOR_WHITE);
//
...
void TM_ILI9341_Puts(uint16_t x, uint16_t y, char *str, TM_FontDef_t *font, uint32_t foreground) {
	...
	while (*str) {
		...
		/* Put character to LCD */
		TM_ILI9341_Putc(ILI9341_x, ILI9341_y, *str++, font, foreground);
	}
}
...
void TM_ILI9341_Puts_background(uint16_t x, uint16_t y, char *str, TM_FontDef_t *font, uint32_t foreground, uint32_t background) {
	...
	while (*str) {
		...
		/* Put character to LCD */
		TM_ILI9341_Putc_background(ILI9341_x, ILI9341_y, *str++, font, foreground, background);
	}
}
